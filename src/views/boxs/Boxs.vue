<template>
  <div>
    ...
    <!--搜索-->
    <div class="my-3">
      <el-input class="w-25" v-model="input" placeholder="请输入用户/盒子名称/编码/KEY" @keyup.enter.native="query"></el-input>
    ...
    methods: {
      ...
      async handleEditShow(boxId) {
        ...
        if (result.data.code === 200) {
          ...
          this.editForm.selectedOptions = [result.data.data.adcode.toString(), result.data.data.cdcode.toString()];
        } else {
          this.$status(result.data.msg);
        }
      },
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let params = {
              ...
              province_id: this.editForm.selectedOptions[0],
              city_id: this.editForm.selectedOptions[1],
            };
