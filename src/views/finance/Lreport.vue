<template>
  <div>
    <div class="navigation">财务管理/财务报表/财务明细</div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      console.log('下级财务明细：', this.$route.query);
      this.getMoneyList();
    },
    methods: {
      async getMoneyList () {
        const result = await userfunDetail(this.$route.query.id, this.input, this.type, this.start_time, this.end_time, this.currentPage, this.pageSize);
        this.loading = false;
      ...
      async exportExcel(){
        ...
        const result = await exportUserfunDetail(this.$route.query.id, this.input, this.type, this.start_time, this.end_time);
